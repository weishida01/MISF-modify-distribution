...
class MISF_train():
    def __init__(self,config):
        ...
        self.inpaint_model = InpaintingModel(in_cha=config.model_in_cha,out_cha=config.model_out_cha).to(torch.device("cuda"))
    ...
    def train(self):
        ...
        while(epoch < self.max_epoch):
            ...
            progbar = Progbar(total, width=10, stateful_metrics=['epoch', 'iter', 'ids'])

            for items in train_loader:
                self.inpaint_model.train()

                img_ins, img_gts ,paths = items
                img_ins = img_ins.to(torch.device("cuda"))
                img_gts = img_gts.to(torch.device("cuda"))

                try:
                    outputs, gen_loss, dis_loss, logs = self.inpaint_model.process(img_ins, img_gts)
                except:
                    print('\n向前传播错误')
                    continue
                ...
                if (self.inpaint_model.iteration - 1) % self.save_eg_intervar == 0:
                    ...
                    save_results(results_path,epoch,self.inpaint_model.iteration,img_ins,img_gts,outputs,paths)
                ...
                img_id = ''
                for path in paths:
                    img_id = img_id + str(path.split('/')[-1].split('.')[0])
                logs = [("epoch", epoch),("iter", iteration),("ids", img_id)] + logs
                self.log(logs)

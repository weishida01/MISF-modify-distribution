...
class InpaintGenerator(BaseNetwork):
    def __init__(self, in_cha=3,out_cha=3,residual_blocks=8, init_weights=True):
        ...
        self.encoder0 = nn.Sequential(
            ...
            nn.Conv2d(in_channels=in_cha, out_channels=64, kernel_size=7, padding=0),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReflectionPad2d(3),
            nn.Conv2d(in_channels=64, out_channels=out_cha, kernel_size=7, padding=0),
        )

        self.kernel_pred = KernelConv(kernel_size=[3], sep_conv=False, core_bias=False)

        self.kpn_model = kpn_utils.create_generator(in_cha,out_cha)

        if init_weights:
            self.init_weights()

    def forward(self, x):    # x n*4*256*256
        inputs = x.clone()
        ...
        x = (torch.tanh(x) + 1) / 2    # tanh 变为-1到1 最后变为0-1

...
class InpaintingModel(BaseModel):
    def __init__(self,in_cha=3,out_cha=3):
        super(InpaintingModel, self).__init__('InpaintingModel')

        # generator input: [rgb(3) + edge(1)]
        # discriminator input: [rgb(3)]
        generator = InpaintGenerator(in_cha=in_cha,out_cha=out_cha)
        discriminator = Discriminator(in_channels=out_cha, use_sigmoid=True)
    ...
    def process(self, img_ins, img_gts):
        self.iteration += 1
        ...
        gen_content_loss1 = self.perceptual_loss(outputs[:, [0, 0, 0], :, :], img_gts[:, [0, 0, 0], :, :])
        gen_content_loss2 = self.perceptual_loss(outputs[:, [0, 0, 0], :, :] * masks_in,img_gts[:, [0, 0, 0], :, :] * masks_in) * 5
        ...
        gen_style_loss1 = self.style_loss(outputs[:, [0, 0, 0], :, :], img_gts[:, [0, 0, 0], :, :])
        gen_style_loss2 = self.style_loss(outputs[:, [0, 0, 0], :, :] * masks_in,img_gts[:, [0, 0, 0], :, :] * masks_in) * 5
        ...
        # # generator l1 loss
        # L1_LOSS_WEIGHT = 40
        # gen_l1_loss1 = self.l1_loss(outputs, img_gts)
        # gen_l1_loss2 = self.l1_loss(outputs * masks_in, img_gts * masks_in) * 500
        # gen_l1_loss3 = self.l1_loss(outputs * masks_gt, img_gts * masks_gt) * 100
        # gen_l1_loss = (gen_l1_loss1 + gen_l1_loss2 + gen_l1_loss3) * L1_LOSS_WEIGHT
        # gen_loss += gen_l1_loss
        #
        #
        # # generator perceptual loss
        # CONTENT_LOSS_WEIGHT = 6
        # gen_content_loss1 = self.perceptual_loss(outputs[:,[0,0,0],:,:], img_gts[:,[0,0,0],:,:])
        # gen_content_loss2 = self.perceptual_loss(outputs[:,[0,0,0],:,:] * masks_in, img_gts[:,[0,0,0],:,:] * masks_in) * 30
        # gen_content_loss3 = self.perceptual_loss(outputs[:,[0,0,0],:,:] * masks_gt, img_gts[:,[0,0,0],:,:] * masks_gt) * 10
        # gen_content_loss = (gen_content_loss1 + gen_content_loss2 + gen_content_loss3) * CONTENT_LOSS_WEIGHT
        # gen_loss += gen_content_loss
        #
        #
        # # generator style loss
        # STYLE_LOSS_WEIGHT = 2000
        # gen_style_loss1 = self.style_loss(outputs[:,[0,0,0],:,:], img_gts[:,[0,0,0],:,:])*10
        # gen_style_loss2 = self.style_loss(outputs[:,[0,0,0],:,:] * masks_in, img_gts[:,[0,0,0],:,:] * masks_in) * 50
        # gen_style_loss3 = self.style_loss(outputs[:,[0,0,0],:,:] * masks_gt, img_gts[:,[0,0,0],:,:] * masks_gt) * 10
        # gen_style_loss = (gen_style_loss1 + gen_style_loss2 + gen_style_loss3) * STYLE_LOSS_WEIGHT
        # gen_loss += gen_style_loss

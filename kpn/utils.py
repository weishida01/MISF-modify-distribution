...
class Config():
    def __init__(self):
        ...
        self.spatial_att = False
        self.upMode = 'bilinear'
        self.core_bias = False
        self.init_type = 'xavier'
        self.init_gain = 0.02


def create_generator(in_cha, out_cha):
    opt_ = Config()

    # Initialize the network
    FILTER_TYPE = '64'
    generator = network.KPN(in_cha, out_cha, opt_.color, opt_.burst_length, opt_.blind_est, opt_.kernel_size,opt_.sep_conv,
                            opt_.channel_att, opt_.spatial_att, opt_.upMode, opt_.core_bias, FILTER_TYPE)

    network.weights_init(generator, init_type=opt_.init_type, init_gain=opt_.init_gain)
...
def save_model(iteration, generator, t=''):
    BATCH_SIZE = 1
    kpn_model_save_path = './result/model'
    model_name = '{}_KPN_bs_{}_{}.pth'.format(iteration, BATCH_SIZE, t)
    save_model_path = os.path.join(kpn_model_save_path)

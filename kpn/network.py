...
class KPN(nn.Module):
    def __init__(self, in_cha=3, out_cha=3, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
        ...
        out_channel_img = out_cha * (self.kernel_size[0] ** 2)
    ...
    def forward(self, data_with_est, x):
        ...
        kernels = F.interpolate(input=kernels,
        ...
        conv8 = self.conv8(
            torch.cat([conv2, F.interpolate(conv7, scale_factor=2, mode=self.upMode, align_corners=True)],
                      dim=1))  # 128*128*128
        conv9 = self.conv9(
            torch.cat([conv1, F.interpolate(conv8, scale_factor=2, mode=self.upMode, align_corners=True)],
                      dim=1))  # 64*256*256
...
class KernelConv(nn.Module):
    ...
    def _sep_conv_core(self, core, batch_size, N, color, height, width):
        ...
        for K in self.kernel_size:
            ...
            core_out[K] = torch.einsum('ijklno,ijlmno->ijkmno', [t1, t2]).view(batch_size, N, K * K, color, height,
                                                                               width)
            cur += K
        # it is a dict
...
class LossFunc(nn.Module):
    ...
    def forward(self, pred_img_i, pred_img, ground_truth, global_step):
        ...
        :return: loss
        """
        return self.coeff_basic * self.loss_basic(pred_img, ground_truth), self.coeff_anneal * self.loss_anneal(
